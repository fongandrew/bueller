...
await assertFileExists('issues/review/p1-001-hello.md', 'FAIL: Issue not moved to review');

// Check that the issue is not in open anymore
await assertFileNotExists('issues/open/p1-001-hello.md', 'FAIL: Issue still in open directory');

// Check that hello.txt was created
await assertFileExists('hello.txt', 'FAIL: hello.txt was not created');
